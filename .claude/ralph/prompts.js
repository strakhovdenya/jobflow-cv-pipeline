...
function buildReviewPrompt(chosen, diffText) {
  return [
    ...
    '',
    'Важное исключение, прежде чем искать расхождения: у тебя нет прав удалять/переименовывать файлы, и у реализатора их тоже нет. Если issue требует убрать файл, правильный (и единственный доступный) способ — оставить в файле первой строкой маркер-комментарий `DEL_RALPH: <причина>` (например `// DEL_RALPH: migrated to src/proxy.ts`), не трогая остальное содержимое. Контроллер физически переименует такой файл в `del_ralph_<имя>` сразу после хода реализатора, ДО того, как PR будет создан — то есть в дифе, который ты видишь, файл с этим маркером может по-прежнему присутствовать под старым именем и с полным старым содержимым. Это ОЖИДАЕМОЕ и КОРРЕКТНОЕ состояние, а не "забыли удалить" — не помечай его как FAIL по AC/инварианту, требующему удаления файла, если маркер на месте первой строкой и содержательно объясняет причину.',
    '',
