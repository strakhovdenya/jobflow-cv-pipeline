...
**TASK-085** (Component: UpcomingStepsPanel) is now DONE (2026-07-31) — see
`project-management/completed-tasks/TASK-085-upcoming-steps-panel.md` and the 2026-07-31
`TEST_LOG.md` entry. Recommended next: **TASK-087** (ActionsPanel), the next not-yet-built
...
TASK-085's own PR has merged into the base branch (ADR-025). Remaining epic components after
TASK-087: TASK-088 (CoverLetterPanel), TASK-089 (TrackingPanel), each followed by wiring their own
real data once built (it's each task's own job, not retroactive). Then **TASK-074** last per the
project owner's explicit request. Process note: branch a new epic sub-task only after the
immediately-preceding sub-task's PR into the base branch has merged (ADR-025).
