...
**TASK-083** (Wire real backend workspace data into the PipelineScreen data contract) is now DONE
(2026-07-30) — see `project-management/completed-tasks/TASK-083-real-backend-data.md` and the
2026-07-30 `TEST_LOG.md` entry. Recommended next: **TASK-084** (Component: ChecksPanel), the next
not-yet-built component in the epic (pre-PDF-check/final-check status), on a new branch off the
same epic base branch `task/TASK-073-redesign-base` — awaiting explicit user selection before
starting. Remaining epic components after TASK-084: TASK-085 (UpcomingStepsPanel), TASK-087
(ActionsPanel), TASK-088 (CoverLetterPanel), TASK-089 (TrackingPanel), each followed by wiring
their own real data once built (TASK-083's board row previously implied that wiring would happen
retroactively as part of TASK-083 itself — corrected: those panels didn't exist yet, so there was
nothing to wire; it's each task's own job), then **TASK-074** last per the project owner's explicit
request. Process note: branch a new epic sub-task only after the immediately-preceding sub-task's
PR into the base branch has merged (ADR-025) — TASK-083's own PR must merge into the base branch
first.
