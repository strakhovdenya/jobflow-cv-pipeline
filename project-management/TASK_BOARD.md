...
`task/TASK-066-pre-pdf-check-ui`. New `apps/web/src/app/workspaces/[id]/pre-pdf-check-panel.tsx`:
...
| TASK-066 | Phase 15 — Full Pipeline Control UI | Add Prompt 3 (pre-PDF check) trigger and results view | DONE | P2 | see docs/07_task_backlog.md | branch task/TASK-066-pre-pdf-check-ui | New `pre-pdf-check-panel.tsx`: trigger button + structured result (readiness/corrections/export_blocked banner/overall_notes) fetched from the registered `pre_pdf_check_json` artifact via the existing download proxy, since the POST response itself only carries `readiness`. 63/63 apps/web tests pass (5 new) |
