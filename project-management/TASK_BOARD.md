...
**TASK-087** (Component: ActionsPanel) is now DONE (2026-08-01) — see
`project-management/completed-tasks/TASK-087-actions-panel.md` and the 2026-08-01 `TEST_LOG.md`
entry. Recommended next: **TASK-088** (CoverLetterPanel), the next not-yet-built component in the
epic, on a new branch off the same epic base branch `task/TASK-073-redesign-base` — awaiting
explicit user selection before starting, and only once TASK-087's own PR has merged into the base
branch (ADR-025). Remaining epic components after TASK-088: TASK-089 (TrackingPanel), each
followed by wiring their own real data once built (it's each task's own job, not retroactive).
Then **TASK-074** last per the project owner's explicit request. Process note: branch a new epic
sub-task only after the immediately-preceding sub-task's PR into the base branch has merged
(ADR-025).
