...
Recommended next: **TASK-069** (Add application tracking actions to workspace detail UI) — next
task of Phase 15, `apps/web`-only.

Last completed: TASK-068 (Add cover letter generation trigger and content view) — DONE, branch
`task/TASK-068-cover-letter-ui`. New `apps/web/src/app/workspaces/[id]/cover-letter-panel.tsx`,
following TASK-067's exact pattern: a "Generate cover letter" button eligible at either
`cv_pdf_generated` or `final_check_ready` (matching `CoverLetterInputBuilderService`'s
`COVER_LETTER_ALLOWED_STATUSES` guard — two statuses, unlike TASK-067's single-status guard).
Per this task's AC, content is not re-rendered by a second bespoke view — the generated
`cover_letter.md`/`cover_letter.json` artifacts are shown through TASK-064's existing
`ArtifactViewer` (already renders every workspace artifact with View/Download), so the panel
itself only shows the trigger and, once a cover-letter artifact exists, a note pointing at the
Artifacts section — using TASK-067's post-review "artifact-existence-driven eligibility" pattern
from the start rather than a hardcoded status whitelist. New `lib/api.ts` `generateCoverLetter()`
+ `actions.ts` `generateCoverLetterAction`, following the exact existing pattern; wired into
`page.tsx` alongside the other pipeline-step panels. `apps/web`-only, no backend changes (the
endpoint pre-existed since TASK-049). New `cover-letter-panel.spec.tsx` (7 tests) covers: not
rendered outside eligible statuses with no result yet, button visible at both eligible statuses,
button hidden but panel/note still shown once status advances past them (as long as the artifact
exists), the success/refresh path, the validation-failure path, and the action-level-error path.
78/78 `apps/web` tests pass (7 new); `tsc`/`lint`/`build` all clean. Manually verified against a
real backend (fake AI provider): drove a workspace `source_saved` → `cv_pdf_generated` through
every intermediate gate, called `generate-cover-letter`, confirmed the `cover_letter_md`/
`cover_letter_json` artifacts registered correctly and workspace status advanced to
`cover_letter_generated`; started the real `apps/web` dev server against this backend and
confirmed via `curl` that the rendered page shows the panel's "available in the Artifacts
section" note (button correctly hidden) and both artifact rows in the Artifacts table, then
fetched the JSON artifact through the frontend's own download proxy and confirmed it matches the
`CoverLetterOutput` shape generated by the backend. No live browser click-through (no browser
automation tool available) — covered instead by the component's tests. Test workspace and its
DB rows/storage folder cleaned up afterward; both dev servers stopped. See
`project-management/TEST_LOG.md` 2026-07-20 TASK-068 entry for full detail.
...
| TASK-068 | Phase 15 — Full Pipeline Control UI | Add cover letter generation trigger and content view | DONE | P2 | TASK-064 | task/TASK-068-cover-letter-ui | New `cover-letter-panel.tsx`: "Generate cover letter" button eligible at `cv_pdf_generated`/`final_check_ready`; content rendered via TASK-064's existing `ArtifactViewer` (no second bespoke view), artifact-existence-driven eligibility per TASK-067's pattern. 78/78 apps/web tests pass (7 new) |
