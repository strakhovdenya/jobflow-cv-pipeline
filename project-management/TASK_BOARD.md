...
Last completed: TASK-PH-009 (Reapply rate limiting onto current main) — DONE, branch task/TASK-PH-009-rate-limiting. `ThrottlerModule.forRootAsync` + global `ThrottlerGuard`, `THROTTLE_TTL`/`THROTTLE_LIMIT` via ConfigService, `GET /health` exempted via `@SkipThrottle()` (user-confirmed); supersedes orphaned TASK-PH-003 branch (now marked SKIPPED).
Recommended next task: **TASK-PH-010** (Add security governance files: SECURITY.md, Dependabot, CodeQL), second of four `Phase PH-2 — Production Hardening Follow-ups` tasks added 2026-07-13 from a user-driven audit; inserted ahead of TASK-044 (see `docs/07_task_backlog.md` §17.2). Recommended order within PH-2: PH-009 → PH-010 → PH-011 → PH-012. Phase 8 (TASK-044) resumes after PH-2 is done.
Current phase: `Phase PH-2 — Production Hardening Follow-ups` (TASK-PH-010..012 remaining, all TODO) inserted ahead of the remainder of `Phase 8 — P1 Safety & Quality Layer` (TASK-044).
