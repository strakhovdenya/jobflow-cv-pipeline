...
first) is now DONE (2026-08-02) — see `project-management/completed-tasks/TASK-074-final-check-after-cover-letter.md`
and the 2026-08-02 `TEST_LOG.md` entry. This was the epic's last implementation sub-task.
Recommended next: **TASK-091** (manual verification pass: re-run TASK-072's real historical flow
variants against the redesigned UI), on a new branch off the same epic base branch
`task/TASK-073-redesign-base` — awaiting explicit user selection before starting, and only once
TASK-074's own PR has merged into the base branch (ADR-025). TASK-091 runs before the epic's final
PR into `main`, per its own sequencing note in `docs/07_task_backlog.md`. Only after TASK-091
confirms parity is the epic's final step taken: one PR from `task/TASK-073-redesign-base` into
`main`. Process note: branch a new epic sub-task only after the immediately-preceding sub-task's PR
into the base branch has merged (ADR-025).
...
(`appliedVia`/`notes` text inputs, `submittedCvArtifactId`/`submittedCoverLetterArtifactId` as a
`<select>` populated from the workspace's own artifact list rather than raw-ID text entry),
`mark-rejected` (`rejectionSummary`/`notes`) and `archive` — previously only reachable via
curl/Swagger. Each sub-section's visibility mirrors `application-tracking.service.ts`'s own
per-action status guard (not a single all-or-nothing panel gate), so e.g. "Mark rejected" only
appears at `status = applied`. New `lib/api.ts` `markReadyToApply()`/`markApplied()`/
...
sub-sections appeared/disappeared at each status, and that the whole panel renders nothing once
`archived`. No live browser click-through (no browser automation tool available) — covered
instead by the component's tests plus the rendered-HTML checks. Test workspace and its DB
rows/storage folder cleaned up afterward. See `project-management/TEST_LOG.md` 2026-07-20
TASK-069 entry for full detail.

Previously: TASK-067 (Add Prompt 5 final check trigger and results view) — DONE, branch
...
| TASK-082 | Phase 15 — Full Pipeline Control UI | Screen: assemble /workspaces list | DONE | P2 | TASK-073 | PR #148 | Done 2026-07-30 — see `project-management/completed-tasks/TASK-082-workspaces-list-screen.md` |
| TASK-083 | Phase 15 — Full Pipeline Control UI | Wire real backend workspace data into the PipelineScreen data contract | DONE | P2 | TASK-081 | branch task/TASK-083-real-backend-data | Done 2026-07-30 — see `project-management/completed-tasks/TASK-083-real-backend-data.md`. Scope was pipeline-view-model.ts's `stages`/`mainCard`/`artifacts` mapping only (per docs/07_task_backlog.md's actual AC/Key Invariants) — most of it already matched real `review-gates.service.ts` preconditions from TASK-081; fixed two real gaps: `analysis_ready` (only reachable as a failed-confirm-skip rollback, was mismapped as a passive "waiting" state) and `failed` (stage position now inferred from real `artifacts[]` instead of hardcoded). This row's older description (wiring real data into `checks`/`upcoming`/`coverLetterPanel`/`trackingPanel`) was aspirational/out of order — those panels (TASK-084/085/087/088/089) don't exist as components yet, so there was nothing to wire; that remains each of those tasks' own job once built. `pipeline-view-model.ts` already renders all 11 stages statically regardless of status, so TASK-074's "final stage disappears" bug (mockup 12) was never reproduced here — nothing to restore |
| TASK-084 | Phase 15 — Full Pipeline Control UI | Component: ChecksPanel (pre-PDF / final check status) | DONE | P2 | TASK-073 | branch task/TASK-084-checks-panel | Done 2026-07-30 — see `project-management/completed-tasks/TASK-084-checks-panel.md`. Pure presentation component rendering two independent optional props: `checks` (not_run / result with readiness/suggestions/blockers/optional findings[]/notes) and `finalCheckPanel` (banner/checks[]/emptySections[]/warnings[]). Findings-list rendering driven strictly by key presence (`'findings' in checks`), not by `compact` or array length, so the mockup-08 `compact: true`+absent-findings case renders no findings section at all while a present-but-empty array shows an explicit "No findings." row. Not wired into `/workspaces/[id]` in this task. 195/195 apps/web tests pass (15 new in `checks-panel.spec.tsx`) |
| TASK-085 | Phase 15 — Full Pipeline Control UI | Component: UpcomingStepsPanel (next-steps summary after PDF export) | DONE | P2 | TASK-073 | branch task/TASK-085-upcoming-steps-panel | Done 2026-07-31 — see `project-management/completed-tasks/TASK-085-upcoming-steps-panel.md`. Pure presentation component rendering `upcoming.finalCheck.status`/`upcoming.coverLetter.status` (arbitrary strings, not the `'Not started'` literal) and `upcoming.tracking.fields[]` as an ordered label list. Not wired into `/workspaces/[id]` in this task. 199/199 apps/web tests pass (4 new in `upcoming-steps-panel.spec.tsx`) |
| TASK-087 | Phase 15 — Full Pipeline Control UI | Component: ActionsPanel (secondary pipeline step-trigger actions) | DONE | P2 | TASK-073 | branch task/TASK-087-actions-panel | Done 2026-08-01 — see `project-management/completed-tasks/TASK-087-actions-panel.md`. Pure presentation component rendering `actionsPanel.title` + `actionsPanel.buttons[]`, reusing a new `ActionButtonRow` component (extracted from `MainActionCard` during same-session `/code-review`, keyed by `` `${label}-${index}` `` instead of the collision-prone `label`-only key) rather than duplicating the button-row JSX. Not wired into `/workspaces/[id]` in this task, no real data mapping (that's TASK-083's job already done, this task just supplies the component). 203/203 apps/web tests pass (4 new in `actions-panel.spec.tsx`) |
...
| TASK-074 | Phase 15 — Full Pipeline Control UI | Fix: final check (Prompt 5) becomes permanently unreachable once cover letter is generated first | DONE | P2 | TASK-067,TASK-068,TASK-083 | branch task/TASK-074-final-check-after-cover-letter | Done 2026-08-02 — see `project-management/completed-tasks/TASK-074-final-check-after-cover-letter.md`. Widened `FINAL_CHECK_ALLOWED_STATUSES` to also allow `cover_letter_generated`, mirroring the cover-letter guard's own symmetric allowance; added an idempotency guard (reject if `05_final_check.json` already exists) since `cover_letter_generated` is terminal in `WorkspaceStatusService.TRANSITIONS` and can't rely on the usual status-leaves-allowed-list one-shot lock; `Prompt5Service` now keeps status at `cover_letter_generated` on success from that entry point instead of regressing to `final_check_ready`. Frontend: `final-check-panel.tsx`'s button now gates on `hasResult` at `cover_letter_generated`; `pipeline-view-model.ts`'s `buildStages` checks real final-check artifact presence for that one status instead of assuming "done" from index position alone (narrowed from a broader per-status check during implementation, and a same-session `/code-review` fix narrowed the artifact check to `final_check_json` only, not `final_check_md` — see the archived task's Progress Notes). 643/643 apps/api tests pass (4 new), 210/210 apps/web tests pass (3 new) |
