...
atomicity and recovery» (PRD pending); **EPIC-28** — dark factory extraction (#491), Ralph core phase;
...
| P1-004 | Medium | No graceful shutdown; in-flight AI jobs are killed and block retries up to 15 min | #406 (shutdown hooks) + to add to #406: `init`, `stop_grace_period`, fail `PromptRun` of stalled jobs |
| P1-005 | Medium | OpenAI SDK retries and 120 s timeout repeat paid calls silently | new issue «LLM provider call policy» (pending) |
| P1-006 | Medium | Artifact versions share one overwritten file; failed regenerate overwrites the reviewed md | EPIC-29 |
| P1-007 | Medium | Full Docker deployment cannot run the pipeline end to end (no artifact volume, no knowledge sources, no Chromium) | new issue «Docker deployment» (pending); owner runs only Postgres/Redis in Docker today |
...
| P2-004 | Medium | Prompt injection via spoofable section headers; forced-claim exemption trusts the AI's own label | #426 (delimiters); forced claims without a manual note — new issue (pending) |
| P2-005 | Medium | Download fails with 500 for Cyrillic file names | #494 |
| P2-006 | Medium | Global throttler keyed by the Next server's single IP trips on job polling; AI enqueue not limited | new issue (pending) |
| P2-007 | Medium | `AI_PROVIDER` silently defaults to `fake` | new issue «LLM provider call policy» (pending) |
| P2-008 | Medium | No output cap, no `finish_reason`/refusal handling, unbounded notes | new issue «LLM provider call policy» (pending) |
| P2-009 | Low | «One active PromptTemplate per step» not enforced by the DB | new issue (pending) |
...
| P2-012 | Low | API responses and prompts expose absolute server paths | new issue (pending) |
| P2-013 | Low | Swagger exposed in the Compose deployment | new issue «Docker deployment» (pending) |
...
| P2-016 | Low | Unpaginated workspace list; no index on `PromptRun.workspaceId` | #406 (pagination); index — to add to #406 |
...
| P3-003 | High | Nothing runs `next build` for `apps/web` (CI and Ralph gate) | new issue for CI (pending); Ralph gate — EPIC-28 |
...
| P3-015 | Low | `/health` is liveness only | new issue «Docker deployment» (pending) |
| P3-016 | Low | e2e does not boot `main.ts`; shares the dev DB locally | new issue (pending) |
