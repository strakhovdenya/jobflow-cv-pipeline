...
import { validateVacancyAnalysisJson } from './vacancy-analysis.schema';

const validJson = JSON.stringify(FAKE_PROMPT1_JSON);

describe('validateVacancyAnalysisJson', () => {
  ...
  it('rejects non-JSON string', () => {
    const result = validateVacancyAnalysisJson('this is not json');
    expect(result.success).toBe(false);
    expect(result.error).toMatch(/not valid JSON/i);
  });

  it('rejects JSON array at root', () => {
    const result = validateVacancyAnalysisJson('[1, 2, 3]');
    expect(result.success).toBe(false);
  ...
  it('accepts all three valid decision values', () => {
    for (const decision of ['apply', 'maybe', 'skip'] as const) {
      const result = validateVacancyAnalysisJson(
